"""unit tests for the hardware.board_position module"""
...
def test_encode(row: int, col: int, expected: int):
    ...
    encoded_pos = BoardPosition(row=row, col=col).encode()

    assert len(encoded_pos) == 1
    assert encoded_pos == expected.to_bytes(1, BoardPosition.ENDIANNESS)
